...
async function upload() {
    ...
    notice.value = { text: '正在导入资助对象名单...', type: 'info' };

    const formData = new FormData();
    formData.append('file', file.value);
    formData.append('academic_year', academicYear.value);

    const response = await fetch('/student-support/import', {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
        notice.value = { text: '导入失败，请确认文件格式为 .xls/.xlsx，且包含“学号”“资助等级”等表头。', type: 'error' };
        return;
    }
    ...
    <main class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <header class="mb-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="text-2xl font-bold text-slate-950">学生资助对象导入</h1>
                    <p class="mt-1 text-sm text-slate-500">导入资助对象名单和资助等级，用于在学生主页显示需要帮助状况。</p>
                </div>
        ...
        <section class="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6">
                    <input type="file" accept=".xls,.xlsx" class="block w-full text-sm text-slate-700 file:mr-4 file:rounded file:border-0 file:bg-slate-900 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white" @change="chooseFile">
                    <p class="mt-3 text-xs text-slate-500">支持 .xls 和 .xlsx。系统会识别“序号、学号、姓名、性别、二级学院、专业、资助等级”格式。</p>
                </div>
                ...
                <div class="mt-5 flex flex-wrap gap-2">
                    ...
                    <a href="/student-support/import/template" class="rounded border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">下载导入示例</a>
                </div>
            ...
            <aside class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 space-y-4 text-sm text-slate-700">
                    <p>第 1 行可为标题，例如“2025-2026学年学生资助对象名单”。</p>
                    <p>第 2 行为表头：序号、学号、姓名、性别、二级学院、专业、资助等级。</p>
                    <p class="text-xs text-slate-500">导入时按“学号 + 学年”更新同一学生的资助等级，避免重复导入。</p>
                </div>
            </aside>
        </section>
    </main>
